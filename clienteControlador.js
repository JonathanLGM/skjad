const { Cliente1 } = require('./db'); // Ahora importa Cliente1
...
const listarClientes = async (req, res) => {
  try {
    const clientes = await Cliente1.findAll(); // Usamos Cliente1
...
const obtenerClientePorId = async (req, res) => {
  try {
    const cliente = await Cliente1.findByPk(req.params.id_cliente); // Usamos Cliente1
